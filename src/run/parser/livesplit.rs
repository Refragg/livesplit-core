...
use {
    ...
    livesplit_auto_splitting::{SettingValue, UserSettingKind},
};
...
fn parse_auto_splitter_settings(
    ...
    parse_children(reader, |reader, tag, attributes| match tag.name() {
        ...
        "Start" => type_hint(text(reader, |t| {
            settings.set_start(parse_bool(t.as_ref()).unwrap_or_default())
        })),
        "Reset" => type_hint(text(reader, |t| {
            settings.set_reset(parse_bool(t.as_ref()).unwrap_or_default())
        })),
        "Split" => type_hint(text(reader, |t| {
            settings.set_split(parse_bool(t.as_ref()).unwrap_or_default())
        })),
        "CustomSettings" => {
            parse_children(reader, |reader, _tag, attributes| {
                ...
                type_hint(parse_attributes(attributes, |k, v| {
                    match k {
                        ...
                        "type" => match v.unescape_str().as_str() {
                            "bool" => custom_setting.set_setting_type(UserSettingKind::Bool {
                                default_value: false,
                            }),
                            _ => custom_setting.set_setting_type(UserSettingKind::Bool {
                                default_value: false,
                            }),
                        },
                        _ => {}
                ...
                type_hint(text(reader, |t| {
                    custom_setting.set_value(SettingValue::Bool(
